...
def conv_audio(fn,ff):
	subprocess.call(f"ffmpeg -i {fn} -loglevel quiet {os.path.splitext(fn)[0]}.{ff}", shell=True)
	rm(fn)
...
for x in range(len(MHZ10)):
	try:
		...
		lstcombs.append([int(round(timebins[segment[0]]*1000)),int(round(timebins[segment[1]]*1000))])
		song = AudioSegment.from_wav("AUDIO.wav")
		extract = song[int(round(timebins[segment[0]]*1000)):int(round(timebins[segment[1]] * 1000))]
